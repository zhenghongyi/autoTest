...
xmlNodePtr CreateNode(NSString* nodeName, NSDictionary<NSString*, NSString*>* attribute) {
    ...
    xmlNodePtr contentNode = xmlNewText(BAD_CAST "\n");
    xmlAddChild(newNode, contentNode);
...
void AddContent(xmlNodePtr node, NSString* content) {
    if (content) {
        char* contentStr = (char *)[content cStringUsingEncoding:NSUTF8StringEncoding];
        xmlNodeAddContent(node, (xmlChar*)contentStr);
    }
}
...
void SurroundNode(xmlNodePtr node, NSString* nodeName, NSDictionary<NSString*, NSString*>* nodeAttribute) {
    ...
    if (nodeAttribute && nodeAttribute.count > 0) {
        SetPropertyForNode(newNode, nodeAttribute);
    }
}

void AddPrevSibling(xmlNodePtr curNode, xmlNodePtr node) {
    xmlAddPrevSibling(curNode, node);
}
